...
from utilities import testmetic
...
class Trainer(object):
    ...
    def __evaluation__(self, eval_loader, eval_iter, epoch, step = 0):
        ...
        with torch.no_grad():
            for _ in tqdm(range(eval_iter)):
                ...
                psnr, ssim = testmetic.calc_metrics(res[0], hr[0], crop_border=self.config["module_params"]["upsampling"])
        ...
        # if (dataset_name == "Urban100") and (final_psnr>self.best_psnr_urban100["psnr"]):
        #     self.best_psnr_urban100["psnr"] = final_psnr
        #     self.best_psnr_urban100["epoch"] = epoch
        saveBestModel = False
        if (dataset_name == "Set14") and (final_psnr>self.best_psnr_set14["psnr"]):
            self.best_psnr_set14["psnr"] = final_psnr
            self.best_psnr_set14["epoch"] = epoch
            saveBestModel = True
        elif (dataset_name == "Set5") and (final_psnr>self.best_psnr_set5["psnr"]):
            self.best_psnr_set5["psnr"] = final_psnr
            self.best_psnr_set5["epoch"] = epoch
            saveBestModel = True
        ...
        if dataset_name == "Set5":
            ...
            print("[{}], Best Set5 PSNR: {:.4f} @ epoch {}".format(self.config["version"],
                                                    self.best_psnr_set5["psnr"], self.best_psnr_set5["epoch"]))
        elif dataset_name == "Set14":
            self.tensorboard_writer.add_scalar('metric/PSNR_Set14', final_psnr, epoch)
            print("[{}], Best Set14 PSNR: {:.4f} @ epoch {}".format(self.config["version"],
                                                    self.best_psnr_set14["psnr"], self.best_psnr_set14["epoch"]))
        ...
        #                                             self.best_psnr_urban100["psnr"], self.best_psnr_urban100         ["epoch"]))
    ...
    def train(self):
        ...
        test_img = torch.rand((1, 3, 320, 180)).cuda()  # 这个是x4的时候用，超分目标是生成1280*720，所以1280/4=320
        #test_img    = torch.rand((1,3,427,240)).cuda()  #X3
        #test_img = torch.rand((1, 3, 640, 360)).cuda()  # 这个是X2的
        ...
        for epoch in range(start, total_epoch):
            for step in range(step_epoch):  # range(step_epoch)
            ...
            if (epoch+1) % model_freq==0:
                # self.__evaluation__(self.eval_loader1, self.eval_iter1, epoch+1)  # Urban100
                # self.__evaluation__(self.eval_loader2, self.eval_iter2, epoch+1)  # B100
                if self.__evaluation__(self.eval_loader3, self.eval_iter3, epoch+1):  # Set14
                    print("Best Set14! Save epoch %d model checkpoint!" % (epoch + 1))
                    #torch.save(self.network.state_dict(),os.path.join(ckpt_dir, 'modelX{}_best_Set14.pth').format(self.config["module_params"]["upsampling"]))
                    torch.save(self.network, os.path.join(ckpt_dir, 'modelX{}_best_Set14.pth').format(self.config["module_params"]["upsampling"]))
                if self.__evaluation__(self.eval_loader4, self.eval_iter4, epoch+1):  # Set5
                    print("Best Set5! Save epoch %d model checkpoint!" % (epoch + 1))
                    torch.save(self.network.state_dict(),
                               os.path.join(ckpt_dir, 'modelX{}_best_Set5.pth'.format(self.config["module_params"]["upsampling"])))