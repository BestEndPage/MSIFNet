...
import utilities.testmetic as testmetic
...
class Tester(object):
    ...
    def __init_framework__(self):
        ...
        # train in GPU
        # if self.config["n_GPUs"] ==2:
        #     os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"
        if self.config["cuda"] >=0:
            self.network = self.network.cuda()
        # if self.config["n_GPUs"] > 1:
        #     self.network = nn.DataParallel(self.network, range(self.config["n_GPUs"]))
        ...
        #model_spec = torch.load_state_dict(model_path)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for iii in tqdm(range(self.test_iter)):
                ...
                for t in range(dataset_size):
                    temp_img = res[t,:,:,:]
                    psnr, ssim = testmetic.calc_metrics(temp_img, hr[t,:,:,:], crop_border=scale)
                    #psnr, ssim = testmetic.calc_metrics(temp_img, hr[t, :, :, :], crop_border=4)
                    ...
                    #cv2.imwrite(os.path.join(save_dir, '{}.jpg'.format(i_name).replace('LRBI', version)), temp_img)
                    total_num += 1
                    total_psnr += psnr
                    total_ssim += ssim
        ...
        print("%d %d"%(final_psnr,total_num))
        print("Elapsed [{}], PSNR: {:.4f}, SSIM: {:.4f}".format(elapsed,final_psnr, final_ssim))
        self.reporter.writeInfo("Plain Checkpoint Epoch: {}, PSNR: {:.4f}, SSIM: {:.4f},time:{}".format(
                                        self.config["checkpoint_epoch"], final_psnr, final_ssim,elapsed))